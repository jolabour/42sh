...
/*   Updated: 2019/02/19 00:59:03 by jolabour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		move_to_right(t_42sh *sh)
{
	...
	if (sh->stdin->line_pos < sh->stdin->len_line)
	{
		if (sh->stdin->cursor_pos % sh->winsize == sh->winsize - 1)
		{
			...
			tputs(tgoto(tgetstr("cr", NULL), sh->winsize, sh->winsize),
					1, putchar_custom);
		}
		else
...
void		move_to_end(t_42sh *sh)
{
	...
	{
		tputs(tgoto(tgetstr("ch", NULL), (sh->stdin->len_line + sh->prompt_len)
					- (i * sh->winsize), (sh->stdin->len_line + sh->prompt_len)
					- (i * sh->winsize)), 0, putchar_custom);
