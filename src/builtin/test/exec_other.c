...
/*   Updated: 2019/04/05 04:34:44 by jolabour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			check_nbr(t_42sh *sh, int *pos)
{
	...
	i = 0;
	if (sh->argv->argv[*pos + 1][i] == '-' || sh->argv->argv[*pos + 1][i] == '+')
		i++;
	while (sh->argv->argv[*pos + 1][i])
	{
		if (sh->argv->argv[*pos + 1][i] > '9' || sh->argv->argv[*pos + 1][i] < '0')
			return (1);
		i++;
	}
	i = 0;
	if (sh->argv->argv[*pos + 3][i] == '-' || sh->argv->argv[*pos + 3][i] == '+')
		i++;
	while (sh->argv->argv[*pos + 3][i])
	{
		if (sh->argv->argv[*pos + 3][i] > '9' || sh->argv->argv[*pos + 3][i] < '0')
			return (3);
		i++;
	}
	return (0);
